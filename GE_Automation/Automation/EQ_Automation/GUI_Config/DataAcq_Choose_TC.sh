...
#############################################  Choose PAARTF DataAcq TC to run  ######################################################################################
zenity_choose_TC=$(
        zenity --radiolist --list --title="Choose PAARTF DataAcq TC " --width="800" --height="750" \
        --column="         Select         " --column="Choose TC to run          " --column="Status" --column="Name"\
        " " "TC 2.1" "In Progress" "Check to see if test pass or failed"\
        " " "XXXXX" "Develop"\
)
...
chmod +x $EQ_Path/PET_Data_Acquisition/TC_2_1/DACQ.1630/DACQ_1630.py
$EQ_Path/PET_Data_Acquisition/TC_2_1/DACQ.1630/DACQ_1630.py
fi
